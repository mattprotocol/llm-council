...
class MemoryService:
    ...
    async def record_episode(
        ...
        if metadata:
            episode_data["metadata"] = json.dumps(metadata)
        ...
        try:
            ...
            if result.get("success"):
                ...
                print(f"[Memory] Failed to record episode: {result.get('error', 'Unknown error')}")
                return False
                
        except Exception as e:
            print(f"[Memory] Error recording episode: {e}")
            return False
    ...
    async def search_memories(
        ...
        """
        Search for related memories in Graphiti.
        ...
        try:
            ...
            print(f"[Memory] Found {len(memories)} related memories")
            return memories
    ...
    async def calculate_confidence(
        ...
        # Format memories for the confidence model
        memories_text = "\n".join([
            f"- [{m['type']}] {m['content']} (created: {m.get('created_at', 'unknown')})"
